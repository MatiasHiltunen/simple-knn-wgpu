...
impl KnnCompute {
    ...
    pub async fn compute_knn(&self, points: &[f32]) -> Result<KnnResult> {
        ...
        let phase1_start = Instant::now();
        {
            ...
            let global_bbox_buffer = self.compute_global_bbox(&mut encoder, &buffers, num_points)?;
    ...
    fn create_buffers(&self, points: &[Point3]) -> Result<ComputeBuffers> {
        ...
        self.context.queue.write_buffer(&num_points_uniform, 0, bytemuck::bytes_of(&num_points));
        ...
        self.context.queue.write_buffer(&config_uniform, 0, bytemuck::bytes_of(&config_data));
    ...
    fn compute_global_bbox(
        ...
        let num_workgroups = ((num_points + 255) / 256).max(1);
        ...
        let reduce_bind_group = self.context.device.create_bind_group(&wgpu::BindGroupDescriptor {
